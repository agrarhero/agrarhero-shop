...
function shell(title, innerHtml, sellerName) {
  ...
  <body style="margin:0;padding:0;background:#eaeee2;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:${INK}">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#eaeee2;padding:30px 12px">
    <tr><td align="center">
      <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="width:600px;max-width:100%;background:#ffffff;border-radius:18px;overflow:hidden;border:1px solid #e0e6d6;box-shadow:0 18px 50px -20px rgba(34,51,31,.28)">
        <tr><td style="padding:26px 34px 20px 34px;background:#ffffff">
          <table role="presentation" cellpadding="0" cellspacing="0"><tr>
            <td style="vertical-align:middle"><img src="${MAIL_LOGO}" width="46" height="46" alt="Agrarhero" style="display:block;border-radius:12px"></td>
            <td style="vertical-align:middle;padding-left:14px">
              <div style="font-size:21px;font-weight:800;letter-spacing:.4px;line-height:1;color:${STEEL}">AGRAR<span style="color:${ACCENT}">HERO</span></div>
              <div style="font-size:10.5px;letter-spacing:.13em;text-transform:uppercase;color:${MUTED};margin-top:5px">Alles für Hof, Stall, Feld &amp; Forst</div>
            </td>
          </tr></table>
        </td></tr>
        <tr><td style="padding:0 34px"><div style="height:3px;border-radius:3px;background:linear-gradient(90deg,${STEEL},${ACCENT} 55%,#8dcc5e)"></div></td></tr>
        <tr><td style="padding:30px 34px 34px 34px">${innerHtml}</td></tr>
        <tr><td style="background:${INK};padding:24px 34px">
          <div style="font-size:15px;font-weight:800;letter-spacing:.5px;color:#ffffff">AGRAR<span style="color:#8dcc5e">HERO</span></div>
          <div style="font-size:12.5px;color:#aab5a0;margin-top:5px;line-height:1.6">Alles für Hof, Stall, Feld &amp; Forst</div>
          <div style="margin-top:8px"><a href="https://agrarhero.de" style="font-size:12.5px;color:#8dcc5e;text-decoration:none;font-weight:600">www.agrarhero.de</a></div>
          <div style="font-size:11px;color:#7f8a76;margin-top:12px">Diese E-Mail wurde automatisch erzeugt.</div>
        </td></tr>
      </table>
    </td></tr>
  </table></body></html>`;
...
function invoiceMail(order, items) {
  ...
  const inner = `
    ...
    <div style="background:#f4f6ec;border-left:4px solid ${ACCENT};border-radius:12px;padding:16px 18px;margin:22px 0;font-size:14px;line-height:1.7">
    ...
    <div style="background:#eef6e7;border:1px solid #cfe6bd;border-radius:12px;padding:16px 18px;margin-top:16px;font-size:14px;line-height:1.7">
      <div style="font-size:14px;font-weight:800;color:${STEEL};margin-bottom:6px">So wird Ihre Bestellung noch schneller bearbeitet</div>
      Wenn Sie den Betrag als <strong style="color:${INK}">Echtzeitüberweisung</strong> (auch SEPA-Instant genannt) anweisen, wird er innerhalb weniger Sekunden auf unserem Konto gutgeschrieben, während eine gewöhnliche Überweisung je nach Bank ein bis zwei Werktage unterwegs sein kann. Dadurch können wir Ihren Zahlungseingang sofort erfassen und Ihre Bestellung umgehend für den Versand vorbereiten. Die Echtzeitüberweisung finden Sie bei nahezu allen Banken direkt im Online-Banking oder in der Banking-App. Wählen Sie beim Überweisen einfach diese Option anstelle der normalen Überweisung.
    </div>
    <div style="background:#fff4e8;border:1px solid #f2d3b6;border-radius:12px;padding:14px 16px;margin-top:16px;font-size:13px;line-height:1.65;color:#8a4b12">
  ...
  const text = `Sehr geehrte/r ${name},\n\nIhre Bestellung ${order.order_number} wurde geprüft und freigegeben. Im Anhang finden Sie Ihre Rechnung ${invNo}.\n\n` +
    ...
    `So wird Ihre Bestellung noch schneller bearbeitet: Wenn Sie den Betrag als Echtzeitüberweisung (SEPA-Instant) anweisen, wird er innerhalb weniger Sekunden auf unserem Konto gutgeschrieben, während eine gewöhnliche Überweisung je nach Bank ein bis zwei Werktage unterwegs sein kann. Dadurch können wir Ihren Zahlungseingang sofort erfassen und Ihre Bestellung umgehend für den Versand vorbereiten. Wählen Sie beim Überweisen im Online-Banking einfach die Echtzeitüberweisung anstelle der normalen Überweisung.\n\n` +
...
function shipmentDelay(order) {
  ...
  const inner = `
    ...
    <p style="color:${MUTED};font-size:14px;margin:0 0 20px">Bestellung <strong style="color:${INK}">${order.order_number}</strong></p>
    <p style="font-size:15px;line-height:1.6">Sehr geehrte/r ${name},<br>
    wir möchten Sie kurz und transparent über den aktuellen Stand Ihrer Bestellung <strong>${order.order_number}</strong> informieren. Die Bearbeitung nimmt derzeit etwas mehr Zeit in Anspruch als gewöhnlich, sodass sich die Versandbereitstellung um wenige Werktage verzögert.</p>
    <div style="background:#f4f6ec;border-left:4px solid ${ACCENT};border-radius:12px;padding:16px 18px;margin:22px 0;font-size:14px;line-height:1.7">
      Aufgrund der aktuell sehr <strong>hohen Nachfrage</strong> benötigt die sorgfältige Zusammenstellung Ihrer Bestellung in unserem Lager momentan etwas mehr Zeit. Die Versandbereitstellung erfolgt daher voraussichtlich <strong>zwei bis drei Werktage später</strong> als üblich. An Ihrer Bestellung wird selbstverständlich bereits gearbeitet.
    </div>
    <p style="font-size:15px;line-height:1.6">Wir bitten die kurze Verzögerung zu entschuldigen und danken Ihnen herzlich für Ihr Verständnis. Sobald Ihre Sendung unser Lager verlässt, erhalten Sie automatisch die Versandbestätigung mit allen Details zur Zustellung.<br><br>
    ${contactHtml(order)}<br><br>
    Mit freundlichen Grüßen<br><strong>Ihr Team von Agrarhero</strong></p>`;
  const text = `Sehr geehrte/r ${name},\n\nwir möchten Sie kurz und transparent über den aktuellen Stand Ihrer Bestellung ${order.order_number} informieren. Die Bearbeitung nimmt derzeit etwas mehr Zeit in Anspruch als gewöhnlich, sodass sich die Versandbereitstellung um wenige Werktage verzögert. `+
    `Aufgrund der aktuell sehr hohen Nachfrage benötigt die sorgfältige Zusammenstellung Ihrer Bestellung in unserem Lager momentan etwas mehr Zeit; die Versandbereitstellung erfolgt voraussichtlich zwei bis drei Werktage später als üblich. An Ihrer Bestellung wird bereits gearbeitet. `+
    `Wir bitten die kurze Verzögerung zu entschuldigen und danken Ihnen für Ihr Verständnis. Sobald Ihre Sendung unser Lager verlässt, erhalten Sie automatisch die Versandbestätigung.\n\n${contactText(order)}\n\nMit freundlichen Grüßen\nIhr Team von Agrarhero`;
...
function paymentReminder(order, items) {
  ...
  const inner = `
    ...
    <div style="background:#f4f6ec;border-left:4px solid ${ACCENT};border-radius:12px;padding:16px 18px;margin:22px 0;font-size:14px;line-height:1.8">
