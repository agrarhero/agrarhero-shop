...
// Assets/Heartbeats/Adminbereich überschreiben NICHT die "aktuelle Seite".
const IGNORE_PATH = /^\/(aktiv|admin|api|favicon|robots|sitemap|css|js|img|images|fonts|assets|static|manifest|apple-touch|site\.webmanifest|\.well-known)/i;

// sid -> { userId: number|null, at: epoch-ms, bot: bool, path: string|null, pathSince: epoch-ms|null }
const map = new Map();

function isBot(ua) { return BOT_RE.test(ua || ''); }

// Bei jedem echten Request/Heartbeat aufrufen. sid = Sitzungs-ID, path = originalUrl.
function touch(sid, userId, ua, path) {
  if (!sid) return;
  ...
  let curPath = prev ? prev.path : null;
  let pathSince = prev ? prev.pathSince : null;
  const clean = path ? String(path).split('#')[0] : '';
  const bare = clean.split('?')[0];
  if (clean && !IGNORE_PATH.test(bare)) {
    // Nur bei echtem Seitenwechsel den "Auf dieser Seite seit"-Zeitpunkt neu setzen.
    if (curPath !== clean) { curPath = clean; pathSince = Date.now(); }
  }
  map.set(sid, { userId: userId != null ? Number(userId) : null, at: Date.now(), bot, path: curPath || null, pathSince: pathSince || null });
}

// Aktuelle Seite eines eingeloggten Kunden (aktivste Sitzung), falls online.
function userCurrent(userId) {
  if (userId == null) return null;
  const uid = Number(userId); const now = Date.now(); let best = null;
  for (const e of map.values()) {
    if (e.userId === uid && (now - e.at) <= ONLINE_WINDOW_MS) { if (!best || e.at > best.at) best = e; }
  }
  if (!best) return null;
  return { path: best.path || null, since: best.pathSince || best.at, at: best.at, online: true };
}

// Aktuelle Seite einer bestimmten Sitzung (für Gäste), falls online.
function sidCurrent(sid) {
  const e = map.get(sid); if (!e) return null;
  if ((Date.now() - e.at) > ONLINE_WINDOW_MS) return null;
  return { path: e.path || null, since: e.pathSince || e.at, at: e.at, online: true };
}
...
module.exports = { touch, logout, snapshot, isUserOnline, userCurrent, sidCurrent, isBot, ONLINE_WINDOW_MS, GUEST_WINDOW_MS };
