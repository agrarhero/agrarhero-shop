...
function record(req) {
  try {
    ...
    if (isPage && (!pages.length || pages[pages.length - 1].p !== path)) {
      pages.push({ p: path, t: now });
...
module.exports = { record, list, saveGeo, prune, parseUa, looksFake, markSuspect, RETENTION_MS };
